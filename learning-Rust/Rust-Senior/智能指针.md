...
智能指针在 Rust 中很常见，本次挑选几个最常用、最有代表性的进行讲解：
...
如果说 `Box<T>` 是不是 Rust 中最常见的智能指针，那估计没有任何争议。因为 `Box<T>` 允许你将一个值分配到堆上，然后在栈上保留一个智能指针指向堆上的数据。
...
> 当我们在程序中创建新的栈帧（stack frame）时，栈指针会向内存的较低地址方向移动。
>
> 具体来说，每当我们在函数中声明一个新的局部变量、传递函数参数或者调用另一个函数时，栈指针都会向下移动一定的距离，以为新的栈帧分配空间。这种栈指针不断向下移动的过程被称为栈的增长。
...
fn main() {
    ...
    let (first, second) = (&arr[0], &arr[1]); // // &Box<{integer}>
...
fn gen_static_str() -> &'static str{
    ...
    Box::leak(s.into_boxed_str())//  将 String 类型的字符串转换为 Box<str> 类型的堆分配字符串。
}
```
...
那么问题来了，Rust 不是号称安全的语言吗？为何允许将 `&&mut Person` 跟 `Person` 进行匹配呢？答案就在本章节中。
...
- **结构体内部，按照顺序的方式**，结构体 `_x` 中的字段按照定义中的顺序依次 `drop`(是 HasTwoDrops 结构体中定义的顺序 )
...
```console
error[E0277]: `Rc<String>` cannot be sent between threads safely //不能在线程间安全的传递
```
...
fn main() {
    ...
    let s1 = s.borrow(); // 获取对值的临时不可变借用
    let s2 = s.borrow_mut();  // 获取对值的临时可变借用
