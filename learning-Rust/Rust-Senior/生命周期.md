...
fn main() {
    let mut foo = Foo;
    let loan = foo.mutate_and_share(); // 可变借用 -> 不可变借用
    foo.share(); // 不可变借用
    println!("{:?}", loan);
}
```
...
fn main() {
        ...
        V: Default,
    {
        match map.get_mut(&key) {
            Some(value) => value,
            None => {
                map.insert(key.clone(), V::default()); // .clone需要进行可变借用
...
之前我们提到了很难解决，但是并没有完全堵死，以下就不失为一个解决方案。
...
```rust
use std::vec::Vec;
fn read_length(strings: &mut Vec<String>) -> usize { // 对调用者的的可变借用 
   strings.len() // .len方法是对于调用者的不可变借用
}
```
...
对于字符串字面量来说，他直接被打包到二进制文件中，永远不会被`drop`,因此它能跟程序活的一样久，自然他的生命周期就是`'static`

但是，**`&'static`生命周期针对的仅仅是引用，而不是持有该引用的变量**，对于变量来说，还是要遵守相应的作用域规则：
...
fn main() {
    ...
    print_it(&i); // 引用当离开这一行的时候它的生命周期就结束了，那么给他指定为 'static 自然就报错了
    print_it1(&i); // 同上
}
```
...
fn print_it<T: Debug + 'static>( input: &T) {  // 这里没用使用到 T 所以对于T并没有进行生命周期的检查
    println!( "'static value passed in is: {:?}", input );
}
...
大家有没有想过，到底是 `&'static` 作用域这个引用还是该引用指向的数据呢？
