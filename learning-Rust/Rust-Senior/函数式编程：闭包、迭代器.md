...
>这是因为 Rust 的闭包类型是根据函数签名、捕获的变量和其生命周期等因素而确定的
...
> 需要注意的是，其实 **Fn 特征**不仅仅适用于闭包，还适用于函数，因此上面的 `query` 字段除了使用闭包作为值外，还能使用一个具名的函数来作为它的值
...
`FnOnce`，该类型的闭包会拿走**被捕获变量**的所有权。**`Once` 顾名思义，说明该闭包只能运行一次**：
...
{
    println!("{}", func(3)); 
    println!("{}", func(4)); // 当第二次调用时，self本身已被消耗掉了，所以报错
}

fn main() {
    let x = vec![1, 2, 3];
    fn_once(|z|{z == x.len()})  // 这里闭包实现的是 Fn trait 并不是 FnOnce trait 所以传入的是 x 的引用
    println!("x={:?}", x) 
}
```
...
> 这里指的所有权应该是 闭包本身的 而不是 外部参数，因为调用时实现的是 Fn 不是 FnOnce
...
let handle = thread::spawn(move || {
    println!("Here's a vector: {:?}", v); // v的所有权移交给 handle 这个闭包
});
handle.join().unwrap();  
println!("{:?}", v); // error 此处 x 已经不存在
```
...
fn main() {
    ...
    let update_string =  |str| s.push_str(str);  // 闭包对象他会自动推导为 impl FnMut(&str) 类型
...
> 闭包的可变性是由 FnMut trait 确定的，而不是由变量声明决定的。在 Rust 中，闭包具有自身的可变性，因此即使 update_string 没有被声明为 mut，它仍然可以作为参数传递给 exec 函数。
...
fn main() {
    ...
    let update_string =  |str| s.push_str(str); // 此处实现的是 impl FnMut(&str)
...
fn exec<'a, F: Fn(&'a str)>(mut f: F)  { // 这里要求是 Fn 
    f("hello")
}
```
...
在上面，我们讲到 `move` 关键字对于 `FnOnce` 特征的重要性，但是实际上使用了 `move` 的闭包依然可能实现了 `Fn` 或 `FnMut` 特征。
...
fn main() {
    ...
    let update_string =  move || println!("{}",s); // 闭包本身是没有实现 Display 的
...
fn main() {
    ...
    exec(update_string);
     ...
     println!("{:?}", s); // error： FnOnce trait 会捕获外部参数的所有权
}
...
rintln!("{}", v1_iter); //error 此处 v1_iter 的所有权已经被 迭代器转移消耗掉了
```
...
> 关联类型是在特征定义的语句块中，申明一个自定义类型，这样就可以在特征的方法签名中使用该类型 如上文的 type Item;
...
> Iterator 是用于对已有序列进行迭代和转换操作的 trait，而 IntoIterator 则是用于将某个类型转换为可迭代的 trait。通常情况下，当你需要对一个已有序列进行迭代操作时，你会使用 Iterator trait；而当你需要将某个类型转换为可迭代的对象时，你会使用 IntoIterator trait。
...
v1.iter().map(|x| x + 1); // map方法接受一个闭包函数作为参数，并返回一个新的迭代器，此处将每个元素+1
```
...
为何 `collect` 在消费(使用)时要指定类型？是因为该方法其实很强大，可以收集成多种不同的集合类型，`Vec<T>` 仅仅是其中之一，因此我们必须显式的告诉编译器我们想要收集成的集合类型。
...
可以看出，实现自己的迭代器非常简单，但是 `Iterator` 特征中，不仅仅是只有 `next` 一个方法，那为什么我们只需要实现它呢？因为其它方法都具有默认实现**(特征章节)**，所以无需像 `next` 这样手动去实现，而且这些默认实现的方法其实都是基于 `next` 方法实现的。
