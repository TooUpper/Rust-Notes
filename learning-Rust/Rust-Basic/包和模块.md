...
对于 Rust 而言，包是一个**独立的可编译单元**，它编译后会生成一个可执行文件或者一个库。
...
pub use crate::front_of_house::hosting; //这样其他模块也通过我们可以访问到该模块或者函数
...
假设我们可以创建一个目录 `front_of_house`，然后在文件夹里创建一个 `hosting.rs` 文件，`hosting.rs` 文件现在就剩下：
